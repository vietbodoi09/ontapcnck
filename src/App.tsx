...
const EN:Record<string,string> = {
  ...
  "252621":"Đề GK 2025.2 - Mã 252621",
  "252624":"Đề GK 2025.2 - Mã 252624",
  "2404":"Đề GK HK2 2024-2025 - Mã 2404",
  "17221":"Đề CK 2017.2 - Mã 17221",
};
...
function ExpBlock({text}:{text:string}) {
  const lines = text.split('\n');
  return <>{lines.map((line,i)=><p key={i} style={i>0?{fontSize:13,margin:'2px 0',opacity:0.92}:{}}>{line}</p>)}</>;
}
  ...
  if(scr==="home") return <>
    ...
    <div className="ctn">
      <div className="ht">
        ...
        <p>{"PGS. TS. Vũ Đình Toại - Trường Cơ khí, ĐHBKHN"}</p>
    ...
    <div className="ft">{`Dựa trên bài giảng PGS. TS. Vũ Đình Toại - ĐHBKHN`}</div>
  </>;
    ...
    return <>
      ...
      <div className="ctn" style={{paddingTop:24,paddingBottom:32,maxWidth:720}}>
        ...
        {sr&&<div className={`ex ${sa===q.correct?"eok":"eno"}`}>
          <div className="eh"><span>{sa===q.correct?CHECK:CROSS}</span><span>{sa===q.correct?"Chính xác!":"Sai rồi!"}</span></div>
          <ExpBlock text={q.explanation}/>
        </div>}
          ...
          return <div key={q.id} className="wi">
            ...
            <div className="wos">{q.options.map((o,i)=>
              <div key={i} className={`wo ${i===q.correct?"wc":i===uans?"ww":"wn"}`}>
                <b style={{width:20}}>{OL[i]+"."}</b> {o}{i===q.correct?(" "+CHECK):""}{i===uans&&i!==q.correct?(" "+CROSS):""}
              </div>
            )}</div>
            <div className="we"><ExpBlock text={q.explanation}/></div>
          </div>;
        })}
      </div>}
    </div>
